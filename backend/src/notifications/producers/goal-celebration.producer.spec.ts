...
import type { GoalCompletedEvent } from "./goal-celebration.producer.js";
import { GoalCelebrationProducer } from "./goal-celebration.producer.js";
...
describe("GoalCelebrationProducer", () => {
  let producer: GoalCelebrationProducer;
  ...
  beforeEach(async () => {
    ...
    producer = module.get<GoalCelebrationProducer>(GoalCelebrationProducer);
  });

  it("uses kind=goal_hit, tier=P2, and the goal_hit dedup key", async () => {
    ...
    expect(outboxInsert).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        dedupKey: `goal_hit:${GOAL_ID}`,
      }),
    );
  });

  it("builds the §4.6 payload with the goal deeplink and en copy", async () => {
    await producer.handle(buildEvent());
  ...
  it("uses french title copy when the user's language is fr", async () => {
    ...
    expect(call[0].payload.title).toBe("Objectif atteint !");
  });
